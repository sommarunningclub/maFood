...
export default async function PdvMenuPage({
  ...
  const { data: pdv } = await supabase
    .from("pdvs")
    .select("id, slug, venue_id, name, category, logo_url, prep_time_min, is_open, is_visible, sells_online, pay_at_counter, instagram_handle, commission_pct, gateway_pct, sort_order, wallet_balance")
    .eq("slug", params.pdv)
